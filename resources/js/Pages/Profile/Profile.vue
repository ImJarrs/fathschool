                        ...
                        <div
                            ...
                            <div class="mb-3">
                                <div class="mb-1 w-full">
                                    ...
                                    <div v-if="image_preview" class="mt-2">
    ...
    watch: {
        avatar_value: {
            ...
            handler(newVal, oldVal) {
                if (newVal.length > 0) {
                    const file = newVal[0].originFileObj;
                    this.form.avatar = file;

                    // make preview from the selected file
        ...
        // update preview when prop changes (after successful save or page reload)
        '$page.props.user.profile_photo_url': function(newUrl) {
            this.image_preview = newUrl;
        },
    },
    methods: {
        submit() {
            console.log('Form avatar before submit:', this.form.avatar);
            console.log('Form data:', {
                name: this.form.name,
                email: this.form.email,
                avatar: this.form.avatar ? 'File: ' + this.form.avatar.name : 'null'
            });

            this.form.post(this.route("profile.update", this.user.id), {
                preserveScroll: true,
                onSuccess: () => {
                    // after save, props should contain new URL; update preview
                    this.image_preview = this.$page.props.user.profile_photo_url;
                    // clear file list so upload component resets
                    this.avatar_value = null;
                }
            });
        },
        browse() {

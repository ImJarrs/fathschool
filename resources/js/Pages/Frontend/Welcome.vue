<template>
    ...
    <FrontendLayout>
        ...
        <div class="flex flex-col lg:flex-row items-start m-8"> 
...
const formattedYouTubeUrl = computed(() => {
    if (props.landing && props.landing.youtube_link) {
        let videoId = '';
        const url = props.landing.youtube_link;

        // Mendukung format link biasa (watch?v=) dan link pendek (youtu.be)
        if (url.includes('v=')) {
        ...
        // Gunakan youtube-nocookie untuk menghindari Error 153 di domain sekolah

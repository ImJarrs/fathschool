<template>
    <AppLayout :title="__('Student Admission')">
                            ...
                            <tr class="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="py-4 px-5">
                                    <div class="flex gap-2">
                                        ...
                                        <button @click="openPasswordModal(user.id)" type="button" class="group relative">
                                            <KeyIcon class="w-6 h-6 text-gray-500 hover:text-gray-400" />
                                            <tool-tip :text="__('Edit Password')" />
                                        </button>
        ...
        <!-- Edit Password Modal -->
        <a-modal v-model:visible="passwordModalVisible" :title="__('Edit Password')" @ok="submitPassword">
            <template #footer>
                <a-button key="cancel" @click="passwordModalVisible = false">
                    {{ __('Cancel') }}
                </a-button>
                <a-button key="submit" type="primary" :loading="passwordLoading" @click="submitPassword">
                    {{ __('Save') }}
                </a-button>
            </template>
            <div class="py-4">
                <label class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                    {{ __('Ketik Password Baru') }}
                </label>
...
import { EyeIcon, MagnifyingGlassIcon, PencilSquareIcon, TrashIcon, KeyIcon } from '@heroicons/vue/24/outline'
...
export default {
    ...
    methods: {
        ...
        submitPassword() {
            if (!this.newPassword) {
                alert(this.__('Password tidak boleh kosong'));
                return;
            }
            
            this.passwordLoading = true;
            this.$inertia.post(this.route('student.update.password', this.selectedUserId), {
